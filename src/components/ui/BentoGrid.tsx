import { cn } from "@/lib/utils";
import React from "react";

/**
 * BentoGrid Component
 * A responsive grid container layout using CSS Grid.
 * 
 * Usage:
 * <BentoGrid>
 *   <BentoGridItem ... />
 *   <BentoGridItem ... />
 * </BentoGrid>
 */
export const BentoGrid = ({
  className,
  children,
}: {
  className?: string;
  children?: React.ReactNode;
}) => {
  return (
    <div
      className={cn(
        "grid md:auto-rows-[18rem] grid-cols-1 md:grid-cols-3 gap-4 max-w-7xl mx-auto",
        className
      )}
    >
      {children}
    </div>
  );
};

/**
 * BentoGridItem Component
 * Individual card item within the BentoGrid.
 * 
 * Supports optional header (image/skeleton), icon, title, and description.
 */
export const BentoGridItem = ({
  className,
  title,
  description,
  header,
  icon,
  onClick,
}: {
  className?: string;
  title?: string | React.ReactNode;
  description?: string | React.ReactNode;
  header?: React.ReactNode;
  icon?: React.ReactNode;
  onClick?: () => void;
}) => {
  return (
    <div
      onClick={onClick}
      className={cn(
        "row-span-1 rounded-xl group/bento hover:shadow-xl transition duration-200 shadow-sm p-4",
        "bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800",
        "justify-between flex flex-col space-y-4",
        onClick && "cursor-pointer",
        className
      )}
    >
      {header}
      <div className="group-hover/bento:translate-x-2 transition duration-200">
        {icon && <div className="mb-2">{icon}</div>}
        {title && (
          <div className="font-sans font-bold text-neutral-600 dark:text-neutral-200 mb-2">
            {title}
          </div>
        )}
        {description && (
          <div className="font-sans font-normal text-neutral-500 text-xs dark:text-neutral-400">
            {description}
          </div>
        )}
      </div>
    </div>
  );
};
